...
### 8k电话音频支持
- 在前端`frontend_conf`中开启`telephony_mode: true`（或将`fs`设置为8000），系统将自动采用电话频带友好的梅尔频带范围（默认低频≈50Hz，高频≈3.8kHz，自动避让奈奎斯特）。
- 也可手动指定：`low_freq`/`high_freq` 来覆盖默认带宽。
- 适用路径：Python推理（`funasr.AutoModel`）、ONNXRuntime前端、libtorch前端、Triton特征提取器。
...
### 性能预设与加速
- 通过 `++profile=latency|balanced|throughput` 一键启用合理的 AMP/TF32/compile 组合。
- 也可手动控制：`++amp=true ++amp_dtype=fp16 ++allow_tf32=true ++compile=true`。
- Python前端若安装`kaldi_native_fbank`，可默认使用高性能实现；否则自动回退。
- `pin_memory`：在 `extract_fbank` 调用链通过 `pin_memory=true` 加速 CPU->GPU 传输（CUDA 环境）。
